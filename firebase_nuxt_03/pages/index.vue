<template>
  <div>
    <div class="container">
      ...
      <button @click="authStore.createUser(email,password)" >Sign up</button>
      <p>You are logged in as {{ credentials?.user?.email }}</p>
      <p>with uid {{ credentials?.user?.uid }}</p>
    </div>
...
/* Add padding and margin to container */
.container {
  margin: auto;
  margin-top: 10%;
  padding: 16px;
